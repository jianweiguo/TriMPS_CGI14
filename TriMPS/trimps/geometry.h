...
namespace Geex {
    ...
    public:
        ...
        vec2g<T> normal() const { return vec2(a,b) ; }
	...
	inline vec2 random_point_tri(vec2& p0, vec2& p1, vec2& p2) {
	...
	inline vec2 random_point_poly(std::vector<vec2>& poly) {
		...
		for(unsigned int i=0; i<n-2; ++i) {
			area.push_back( total+0.5*det(poly[i+1]-poly[0], poly[i+2]-poly[0]) );
		//	double temp = 0.5*det(poly[i+1]-poly[0], poly[i+2]-poly[0])  ;
		//	double tarea = triangle_area(poly[0], poly[i+1], poly[i+2]) ;
			total = area[i] ;
		}
	...
	*/
	inline int binary_search(std::vector<double>& value, int s, int e, double a)  {
		int mid = (s+e)/2 ;
		if(s==e) {
			return s ;
		}
	...
	inline int random_polygon_by_area(std::vector<double>& value, double a) {
		return binary_search(value, 0, value.size()-1, a) ;
	}

	inline bool same_side(const vec2& p, const vec2& q, const vec2& r, const vec2& s) {
		vec2 dir = q - p ;
		return gx_sgn(det(dir, r-p)) == gx_sgn(det(dir, s-p)) ;
	}
    template <class T> inline vec2g<T> gx_min(vec2g<T>& x1, vec2g<T>& x2) {
		return vec2g<T>(gx_min(x1.x, x2.x), gx_min(x1.y, x2.y)) ;
    }
    template <class T> inline vec2g<T> gx_max(vec2g<T>& x1, vec2g<T>& x2) {
		return vec2g<T>(gx_max(x1.x, x2.x), gx_max(x1.y, x2.y)) ;
    }
