...
namespace Geex {
    ...
    void Polygon2::load(const std::string& filename) {
        ...
        std::cerr << "Polygon2: loaded " << size() << " edges" << ", area " << area() << std::endl ;
    }
	...
	double Polygon2::area() {
		double area = 0 ;
		for(unsigned int i=0; i<size() ;++i) {
			area += 0.5*det((*this)[i].vertex[0], (*this)[i].vertex[1]) ;
		}
		return fabs(area) ;
	}

	//New Add
	void Polygon2::getBoundingBox(){
		bbox[0]=bbox[2]=-1e3, bbox[1]=bbox[3]=1e3;
		//we should opimize the code here because every vertex visited twice 
		for(unsigned int i=0; i<size(); i++) {
			for(int j=0;j<2;j++){
				if((*this)[i].vertex[j].x >= bbox[0]){
					bbox[0]=(*this)[i].vertex[j].x;
				}
				if((*this)[i].vertex[j].x < bbox[1]){
					bbox[1]=(*this)[i].vertex[j].x;
				}
				if((*this)[i].vertex[j].y >= bbox[2]){
					bbox[2]=(*this)[i].vertex[j].y;
				}
				if((*this)[i].vertex[j].y < bbox[3]){
					bbox[3]=(*this)[i].vertex[j].y;
				}
			}
		}
	}

	double Polygon2::max(double a,double b)
	{
		return a>b?a:b;
	}
	double Polygon2::min(double a,double b)
	{
		return a<b?a:b;
	}


	bool Polygon2::onBounary(const vec2& p){
		/*for(unsigned int i=0; i<size(); i++) {
		double s=orient((*this)[i].vertex[0], (*this)[i].vertex[1], p);
		if(s==0){
		return true;
		}
		}
		return false;*/
		for(unsigned int i=0; i<size(); i++) {
			double temp=((*this)[i].vertex[1].x-(*this)[i].vertex[0].x)*(p.y-(*this)[i].vertex[0].y)-((*this)[i].vertex[1].y-(*this)[i].vertex[0].y)*(p.x-(*this)[i].vertex[0].x);
			/*temp存储向量AB AC的叉积*/        
			if(temp!=0 || (p.x>max((*this)[i].vertex[0].x,(*this)[i].vertex[1].x) || p.x<min((*this)[i].vertex[0].x,(*this)[i].vertex[1].x)
				|| p.y>max((*this)[i].vertex[0].y,(*this)[i].vertex[1].y) || p.y<min((*this)[i].vertex[0].y,(*this)[i].vertex[1].y)))
				/*如果叉积不为 0或点在矩形外时，则可以判断点C不在线段AB上*/        
			{
				continue;
			}
			else        
	...
	bool Polygon2::segement_intersect(vec2 p,vec2 q){
		//std::ofstream file("aa.txt");
		//int count=0;
		for(unsigned int i=0; i<size(); i++) {
			if(seg_seg_intersect(p, q, (*this)[i].vertex[0], (*this)[i].vertex[1])) {
				return true;
			}
			//if(cross(C,D,B)*cross(C,D,A)<=0&&cross(A,B,C)*cross(A,B,D)<=0&&(cross(A,B,B)!=0&&cross(A,B,D)!=0))
			//if(cross((*this)[i].vertex[0],(*this)[i].vertex[1],q)*cross((*this)[i].vertex[0],(*this)[i].vertex[1],p)<=0 && 
			//		cross(p,q,(*this)[i].vertex[0])*cross(p,q,(*this)[i].vertex[1])<=0 && cross(p,q,(*this)[i].vertex[0])!=0 && cross(p,q,(*this)[i].vertex[1])!=0)
			//{
			//		/*file<<p.x<<" "<<p.y<<std::endl;
			//		file<<q.x<<" "<<q.y<<std::endl;
			//		file<<(*this)[i].vertex[0].x<<" "<<(*this)[i].vertex[0].y<<std::endl;
			//		file<<(*this)[i].vertex[1].x<<" "<<(*this)[i].vertex[1].y<<std::endl;
			//		count++;*/
			//		return true;
			//}
		}
	...
	double Polygon2::cross(vec2 a,vec2 b,vec2 c)
	{
		//计算 向量 ab X ac 
